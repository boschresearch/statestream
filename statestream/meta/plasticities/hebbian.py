...
from statestream.meta.neuron_pool import np_shm_layout, np_state_shape
...
def plast_shm_layout(name, net, param):
    """Function returns shm layout for hebbian plasticity.
    """
    ...
    opt_shm_layout_fct = getattr(importlib.import_module("statestream.meta.optimizer"), optimizer + "_shm_layout")
    opt_shm_layout = opt_shm_layout_fct(name, net, param)
    # Get / set optimizer dependent layout.
    for T in ["parameter", "variables"]:
        for d,d_l in opt_shm_layout[T].items():
            shm_layout[T][d] = d_l

    # Parameter for hebbian learner.    
    # -------------------------------------------------------------------------
