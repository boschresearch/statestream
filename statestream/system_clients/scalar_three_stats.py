...
class scalar_three_stats(object):
    ...
    def __init__(self, name, net, client_param):
        ...
        self.stats = np.zeros([3, len(self.sv)], dtype=np.float32)
        self.window = np.zeros([self.parameter["window"], 3, len(self.sv)],
                               dtype=np.float32)
    ...
    def update_frame_writeout(self):
        ...
        for i in range(len(self.stat_list)):
            s = 'stat ' + self.stat_list[i]
            for v in range(len(self.sv)):
                C = self.sv[v][0]
                V = self.sv[v][1]
                self.stats[i,v] = array_property(self.sv_dat[C][V], self.parameter[s])
        ...
        if self.current_frame % self.parameter["window"] == 0:
            ...
            self.dat['variables']['stats'][:] \
                = np.roll(self.stats_hist, self.parameter["nodes"] - I - 2, axis=2)
            self.window *= 0.0

            self.dat['variables']['stats'][:,:, self.current_frame // self.parameter["window"]] \
                = np.mean(self.window, axis=0)[:,:]
